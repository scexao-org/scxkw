...
def load_table() -> pandas.DataFrame | None:
    if PROCESS_DB_PATH.exists():
        return pandas.read_csv(PROCESS_DB_PATH)
    else:
...
def load_deletion_table() -> pandas.DataFrame | None:
    if PROCESS_DELETION_DB_PATH.exists():
        return pandas.read_csv(PROCESS_DELETION_DB_PATH)
    else:
        logger.warning("WARNING: No deletion CSV found!")
        return None


def check_for_new_folders(directory: pathlib.Path=PROCESS_DIR):
    ...
    if table is None:
        dataframe.to_csv(PROCESS_DB_PATH, mode="w", index=False)
    else:
        dataframe.to_csv(PROCESS_DB_PATH, mode="a", index=False, header=False)
...
def crosscheck_scexao6_sdata(directory: pathlib.Path):
    ...
    sc6_folder = pathlib.Path(f"/mnt/sdata/{directory.name}/vgen2")
    ...
    table.to_csv(PROCESS_DB_PATH, index=False)
