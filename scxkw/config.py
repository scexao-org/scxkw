...
try:
    FITS_HEADER_PATH = os.environ[
        'MILK_SHM_DIR'] + '/fits/'  # Straight to the tmpfs
except:
...
) -> tuple[Redis, typ.Literal[True]] | tuple[None, typ.Literal[False]]:
    try:
        from scxkw.config import REDIS_DB_HOST, REDIS_DB_PORT, GEN2HOST
        from scxkw.redisutil.typed_db import Redis
        return Redis(host=REDIS_DB_HOST, port=REDIS_DB_PORT), True
    except:
        return None, False
