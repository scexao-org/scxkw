...
class Pipeline(redis.client.Pipeline):
    ...
    def __init__(self,
                 ...
                 shard_hint,
                 *,
                 auto_execute: typ.Optional[int] = 50) -> None:
        super().__init__(connection_pool, response_callbacks, transaction,
                         shard_hint)
    ...
    def hset(self,
        ...
        new_mapping = None if mapping is None else {
            k: to_redis_scalar_cast(mapping[k])
            for k in mapping
        }

        return redis.client.Pipeline.hset(self,
                                          name=name,
                                          key=key,
                                          value=new_value,
                                          mapping=new_mapping)
...
class Redis(redis.Redis):
    ...
    def pipeline(self,
                 transaction=False,
                 shard_hint=None,
                 auto_execute: typ.Optional[int] = 50):
    ...
    def hset(self,
        ...
        return redis.Redis.hset(self,
                                name=name,
                                key=key,
                                value=new_value,
                                mapping=new_mapping)

    def redis_batched_hget(
            self, keys: typ.Iterable[str]) -> typ.Mapping[str, ScxkwValueType]:
        with self.pipeline() as pipe:
            for key in keys:
                redis.hget(key, 'value')
            all_values = pipe.execute()
        return {k: v for (k, v) in zip(keys, all_values)}
