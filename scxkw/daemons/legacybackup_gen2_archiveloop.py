...
def gen2_archive(frameids, directory):
    ...
    for i in range(len(frameids)):
        g2proxy.executeCmd(
            'SCEXAO', 'foo', 'archive_fits', [],
            dict(frame_no=frameids[i],
                 path="%s/%s.fits" % (directory, frameids[i])))
...
if __name__ == "__main__":
    ...
    directory = "/mnt/tier1/ARCHIVED_DATA"
    notdone = True
    ...
    while True:
        ...
        if os.path.exists(directory2):
            ...
            for i in range(len(monitored)):
                ...
                cubelist = glob.glob("%s/%slog/%s*.fits" %
                                     (directory2, monitored[i], monitored[i]))
                cubelist.sort()
                ncubes = len(cubelist)
                ntot += ncubes
                print(ntot)
                ...
                if ncubes > 0:
                    ...
                    frameids = gen2_getframeids(g2proxy, camids[monitored[i]],
                                                ncubes)
                    ...
                    for j in range(ncubes):
                        ...
                        while notdone:
                            try:
                                fits.setval(cubelist[j],
                                            "FRAMEID",
                                            value=frameids[j],
                                            savecomment=True)
                                fits.setval(cubelist[j],
                                            "EXP-ID",
                                            value=frameids[j].replace("B","E",1),
                                            savecomment=True)
                                notdone = False
                            except:
                                time.sleep(1)
                        ...
                        # Log the name change
                        with open("%s/name_changes.txt" % (pathc, ),
                                  "a") as namelog:
                            namelog.write("%s\t%s \n" % (oldname, newname))
                            namelog.close()
                        # Archive file
                        g2proxy.executeCmd(
                            'SCEXAO', 'foo', 'archive_fits', [],
                            dict(frame_no=frameids[j],
                                 path="%s/%s" % (pathc, newname)))
