#
# FIRSTPL.py -- FIRSTPL personality for g2cam instrument interface
#
#
"""
This file implements a FIRSTPL instrument interface for Subaru Gen2.
...
the subdirectory like this:
    ...
    $ g2cam --cam=FIRSTPL --gen2host=<gen2host> --loglevel=20 --stderr
...
scripts like:
   ...
   exec FIRSTPL archive_fits frame_no=&get_f_no[FIRSTPL A] path=/path/to/file.fits

Looking at the methods below the comment "FIRSTPL INSTRUMENT COMMANDS"
...
class FIRSTPL_Error(CamCommandError):
    pass

class FIRSTPL(BASECAM):

    def __init__(self, logger, env, ev_quit=None):

        super(FIRSTPL, self).__init__()

        self.logger = logger
        self.env = env
        self.ev_quit = ev_quit
        self.insname = 'FIRSTPL'
    ...
    def initialize(self, ocsint):
        ...
        '''
        super(FIRSTPL, self).initialize(ocsint)
    ...
    def start(self, wait=True):
        super(FIRSTPL, self).start(wait=wait)

        self.logger.info('FIRSTPL ocs interface started.')
    ...
    def stop(self, wait=True):
        super(FIRSTPL, self).stop(wait=wait)
        ...
        self.logger.info("FIRSTPL ocs interface stopped.")
    ...
    def archive_fits(self, frame_no=None, path=None, tag=None):
        ...
        path: str
            the local path on the FIRSTPL computer for the file
        """

        if frame_no is None:
            raise FIRSTPL_Error("Missing frame number!")

        # Check frame_no
        match = re.match('^(\w{3})(\w)(\d{8})$', frame_no)
        if not match:
            raise FIRSTPL_Error("frame_no: '%s' doesn't match expected format" % (frame_no))

        if path is None:
            raise FIRSTPL_Error("Missing path!")

        if not os.path.exists(path):
            raise FIRSTPL_Error("Path (%s) does not refer to a file!" % (path))
    ...
    def get_frames(self, frtype='A', num=1, tag=None):
        ...
        #framelist = [('FPLA%08d' % i) for i in range(num)]

        # write frame list to file
        with open("/tmp/frames_FPL%s.txt" % (frtype,), 'w') as out_f:
            out_f.write('\n'.join(framelist))
