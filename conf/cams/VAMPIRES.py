#
# VAMPIRES.py -- VAMPIRES personality for g2cam instrument interface
#
#
"""
This file implements a VAMPIRES instrument interface for Subaru Gen2.
...
the subdirectory like this:
    ...
    $ g2cam --cam=VAMPIRES --gen2host=<gen2host> --loglevel=20 --stderr
...
scripts like:
   ...
   exec VAMPIRES archive_fits frame_no=&get_f_no[VAMPIRES A] path=/path/to/file.fits

Looking at the methods below the comment "VAMPIRES INSTRUMENT COMMANDS"
...
class VAMPIRES(BASECAM):
    ...
    def archive_fits(self, frame_no=None, path=None, tag=None):
        ...
        path: str
            the local path on the VAMPIRES computer for the file
        """
        ...
        if not os.path.exists(path):
            raise VAMPIRES_Error("Path (%s) does not refer to a file!" % (path))
